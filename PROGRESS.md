...
**Verification:** `npm run build` clean. `npx eslint .` 18 → 19 errors — the one new error is `react-hooks/set-state-in-effect` on the new populate effect, the same rule already present at [`CaseView.jsx:146`](src/pages/CaseView.jsx:146) for its analogous pattern. **The two standing live-UI checks (blank case number/charge round-trips through the client list; a cleared incident date persists after reload) were not run this session** — they require signing into the production app, which was skipped this session rather than have the assistant enter a password. Both were exercised as part of the `b8fcde4`/`c545b81` work on 2026-07-28 (see the nullable-field entry above); re-run before relying on them again if meaningful time has passed.
