...
| `is_pv` | boolean | **NOT NULL, default `false`.** Marks the case as a **probation violation** rather than a charged offense. Schema added ahead of the front-end; wired up 2026-08-19. When true the case carries **no** `charge`, `charge_abbrev`, `classification`, `bond_amount` or `release_status` — the Add Case form hides those fields and writes them as explicit nulls — and every display site renders **"[case number] - PV"** in place of the charge/classification text. `status` still defaults to "open" as normal. **Not indexed in Dexie** (the `cases` store is `'id, incident_id, case_number'`), so it needed no version bump — same as `release_status` and `classification` before it. |
| `pv_sentence` | text | nullable — what the client faces **if probation is revoked**. Meaningful only when `is_pv` is true. Optional by design and frequently left blank at creation, which is why it is also editable on Case View (its own "Sentence (if known)" section above Notes, shown for every PV case, not only ones that already have a value — otherwise a blank one could never be filled in later). Blank saves as `null`, never `''`. Added with `is_pv`; wired up 2026-08-19. |
...
**5. "Courtroom wait time"** added to the shared `DESCRIPTION_OPTIONS`, positioned **between "Initial client meeting" and "Met with ADA"**. This **intentionally breaks the otherwise-alphabetical run, by request** — it belongs beside the other in-court entries. Commented in place so it does not get "fixed" later. It deliberately carries **no** default hours (see #7).
