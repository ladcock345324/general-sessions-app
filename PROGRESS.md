...
- **RLS:** Enabled on all 8 tables (the 7 documented ones plus the stray `hours_backup_causey_20260916` backup table, secured the same day it was discovered — see Known Issues). ~~Each table has an "authenticated users only" policy applied to all commands. The policy expression was updated **2026-06-24 via MCP** to wrap the auth call in a subquery — `USING ((select auth.role()) = 'authenticated')` (was `USING (auth.role() = 'authenticated')`) — so Postgres evaluates `auth.role()` once per query instead of once per row.~~ **SUPERSEDED 2026-09-21:** all 8 tables' policies were rewritten again, this time from role-scoped to **user-scoped** — `USING ((select auth.uid()) = 'ea7ec416-3d46-4714-a188-c0f50a63ddcb')`, Lucas's account UUID. See [Supabase RLS — Stray Backup Table Secured + All Policies Rewritten to User-Scoped (2026-09-21)](#supabase-rls-stray-backup-table-secured-all-policies-rewritten-to-user-scoped-2026-09-21) for the full change and its trade-off. This cleared the "Auth RLS Initialization Plan" performance advisor on the original 7 tables back on 2026-06-24 (see Known Issues) and remains true under the new policy expression. RLS was applied to `clients`, `incidents`, `cases`, `hours`, `next_events` at some prior point; applied to `courtroom_documents` and `personal_notes` on 2026-06-17 via Supabase migration (see `supabase_migration_enable_rls_courtroom_personal_notes.sql`).
- **Auth:** Email/password. One user account. **Self-signup DISABLED (2026-07-23)** — "Allow new users to sign up" is OFF in Auth → Sign In / Providers. This was originally a load-bearing security control because RLS policies were role-scoped (`(select auth.role()) = 'authenticated'`), so ANY authenticated account would have had full read/write on all tables. ~~With the anon key public in the client bundle, leaving signup enabled would allow anyone to self-register and read all client data.~~ **As of 2026-09-21 the RLS policies are `auth.uid()`-scoped to Lucas's account specifically**, not merely role-scoped — see the entry linked above — so a second signed-up account would no longer inherit data access by default. Self-signup stays OFF regardless: this app has exactly one intended user. Do not re-enable. New accounts, if ever needed, are created manually via Auth → Users → Add user.
...
### Supabase RLS — Stray Backup Table Secured + All Policies Rewritten to User-Scoped (2026-09-21)

Two Supabase security-advisor fixes, applied directly via the Supabase MCP connector in the main chat — no app code touched, no build/lint needed, nothing here goes through the normal commit/push flow.

**(a) Stray backup table exposed with RLS disabled.** `hours_backup_causey_20260916` — 52 rows, an apparent manual backup of one client's hours entries, not part of the documented schema (see Database Schema above) and not referenced anywhere in this app's code — turned up in the Supabase security advisor with **CRITICAL** "RLS Disabled in Public": readable and writable by anyone holding the public anon key, no login required, same category of exposure as the `courtroom_documents`/`personal_notes` finding from [RLS Enabled on All Tables (2026-06-17)](#rls-enabled-on-all-tables-2026-06-17). **Fixed** by enabling RLS and adding the same "authenticated users only" policy pattern every other table had at the time — then immediately re-scoped to `auth.uid()` by fix (b) below, so it ends this entry on the same user-scoped policy as the other 7 tables. **Open follow-up, not addressed by this fix:** whether to keep or drop this table entirely is still undecided — flagged under Known Issues.

**(b) All 8 tables' policies rewritten from role-scoped to user-scoped.** The 7 documented tables plus `hours_backup_causey_20260916` above previously used `USING ((select auth.role()) = 'authenticated')` — true for *any* authenticated account. Rewritten to `USING ((select auth.uid()) = 'ea7ec416-3d46-4714-a188-c0f50a63ddcb')`, Lucas's account UUID (`lucasadcock@gmail.com`), confirmed as the sole row in `auth.users` before the change. This closes the Known Issue tracked since the RLS work began: the role-scoped policies meant any authenticated account — not just Lucas's — would have full read/write on all data. The risk was contained (self-signup disabled, single account existing) but was a single point of failure, not a real guarantee.

**Trade-off introduced:** the UUID is now hardcoded into 8 policies. If that auth account is ever deleted and recreated, every policy needs updating with the new UUID or access breaks entirely — there is no longer an indirection through "any authenticated user."

**Verified:** Supabase security advisor shows **zero ERROR/CRITICAL findings**. Only the pre-existing "Leaked Password Protection Disabled" WARN remains, untouched by this work (see Known Issues).
...
**Verification:** `npm run build` clean (only the pre-existing >500 kB chunk notice). `npx eslint .` still **20 errors**, unchanged — none in `ClientFile.jsx` or `ClientFile.module.css`.

✅ **VERIFIED ON-DEVICE 2026-09-21** — confirmed by the user on the production URL: the Personal Notes textarea now auto-grows to show its full content instead of scrolling internally.
...
**What it does now.** The client list restores its scroll position on return — both from a client file and across a full page reload. Every other route resets to the top on entry. Nothing else in the app touches scroll.

#### The two root causes, plainly

Neither was the thing originally suspected, and both are worth remembering:

**1. Nothing scrolled to the top on forward navigation — anywhere in the app.** There was not a single `scrollTo` in `src/pages/` before this work. ⚠️ **Desktop browsers do this for you, which masked it completely**; mobile Safari instead carries the previous page's offset into the new route. A client file is shorter than a scrolled client list, so the carried offset clamped to the end and landed the user at the **bottom of the file they had just opened**. Fixed with `useScrollToTopOnMount()` on every route except the client list.

**2. A clamped restore scroll was persisted as if it were a user scroll.** The save-side listener could not tell our own `window.scrollTo` from a finger. During the restore's reach phase the rows have not arrived, the document is one viewport tall, and the scroll is **clamped — usually to 0**. That clamped scroll fires a `scroll` event, which was recorded and written to storage 150ms later, destroying the saved position.

> ⚠️ **The consequence that made this so hard to see: the failure surfaced one navigation AFTER the trip that caused it.** The trip that corrupted the value still restored correctly, because the hold went on to reach the real target. It was the *next* return that read `0`, skipped the restore and landed at the top. That is why it looked random, untied to any particular client, and roughly one-in-three. Fixed by ignoring scroll events while `isScrollHoldActive()`.

#### The knobs, by name and file

| Knob | File | What it does |
|---|---|---|
| `REACH_DEADLINE_MS` (1500) | [`src/scrollRestore.js`](src/scrollRestore.js) | How long the restore keeps trying while the document is still growing. ⚠️ **Wall clock, deliberately not a frame count** — an IndexedDB read on iOS routinely outlasts any sane frame budget, and the previous 10-frame (~166ms) version gave up before a single row had rendered. Raise it if rows ever get slower to arrive. |
| `SETTLE_MS` (1000) | [`src/scrollRestore.js`](src/scrollRestore.js) | How long the position is held and re-applied after it first lands, so mobile Safari cannot override it afterwards. **Lower it if a deliberate scroll ever gets pulled back** shortly after arriving on the list; that would be the hold overreaching. |
| `SCROLL_TOLERANCE` (2px) | [`src/scrollRestore.js`](src/scrollRestore.js) | How close counts as landed. Exists for fractional offsets on hi-DPI/zoomed displays. |
| The 150ms save throttle | [`src/pages/ClientList.jsx`](src/pages/ClientList.jsx) | How often a scroll position is written while scrolling. Not implicated in any of the bugs. |

**Where the pieces live:** [`scrollRestore.js`](src/scrollRestore.js) is the browser-free decision logic (the REACH/SETTLE state machine, testable without a browser); [`scrollHold.js`](src/scrollHold.js) is the imperative driver plus `useScrollToTopOnMount()`; the client list's save side and its exit captures are in [`ClientList.jsx`](src/pages/ClientList.jsx).

#### Diagnostic scaffolding removed (2026-09-04)

The per-hold outcome log added while chasing this — the capped `gsapp:scrollHoldLog` sessionStorage entry, `readScrollHoldLog()`, and the bookkeeping that fed them — **is gone.** It was scaffolding for a closed bug and nothing ever read it. **Everything load-bearing was kept**: the single-hold token and pre-emption, the per-frame `finished` check, the wall-clock deadline, the settle window, the user-input abort, and the save-side guard. `finish()` no longer takes a `reason`, since the reason only ever fed the log.

> ✅ **Checked repo-wide: no other diagnostic flag or on-screen readout exists.** An earlier plan for a readout behind a constant was never built — the only `debug`/`readout`/`diagnostic` matches anywhere in `src/` were `scrollHold.js`'s own comments, now also gone.
...
- ~~**Every hold now records its outcome** into a capped 10-entry `sessionStorage` log (`gsapp:scrollHoldLog`).~~ — **REMOVED 2026-09-04** once the bug was confirmed fixed on-device. It was diagnostic scaffolding, nothing ever read it, and `finish()` no longer takes a `reason`. The behaviour it sat alongside — the single-hold token, the per-frame `finished` check — is all retained.
...
src/
  ...
  scrollHold.js            # holdScrollAt() + useScrollToTopOnMount() + isScrollHoldActive() — drives and HOLDS a scroll position; one hold live app-wide, newer pre-empts older (2026-09-03)
...
- ~~**RLS policies are role-scoped, not user-scoped.** All 7 tables allow full access to any authenticated user. Safe only while self-signup is disabled and a single account exists — self-signup is confirmed DISABLED as of 2026-07-23 (Auth → Sign In / Providers), so the single-account precondition is actively enforced, not merely assumed (see Supabase Project → Auth). Must be rewritten to `auth.uid()`-scoped policies before any second user or external sharing.~~ **RESOLVED 2026-09-21:** all 8 tables' policies (the 7 original plus the newly-secured `hours_backup_causey_20260916`) were rewritten to `auth.uid()`-scoped, restricting access to Lucas's account UUID specifically rather than any authenticated account. See [Supabase RLS — Stray Backup Table Secured + All Policies Rewritten to User-Scoped (2026-09-21)](#supabase-rls-stray-backup-table-secured-all-policies-rewritten-to-user-scoped-2026-09-21). **New trade-off introduced by the fix:** the UUID is hardcoded into 8 policies — if that auth account is ever deleted and recreated, every policy needs updating with the new UUID or access breaks entirely.
- **`hours_backup_causey_20260916` — retention undecided.** A stray table (52 rows, apparent manual backup of one client's hours entries), not part of the documented schema and not referenced by any app code. Discovered exposed with RLS disabled (CRITICAL advisory finding) and secured on 2026-09-21 — see the entry above — but whether to keep it or drop it entirely was not decided as part of that fix. Revisit and either document it properly or drop it.
...
**Hardcoded credentials in `src/supabaseClient.js`:** The Supabase URL and anon key are committed to the repo. The anon key is designed to be public (it is the client-facing key, not the service-role key). Supabase's security model assumes the anon key is visible to users — it is not a secret. The real guard is RLS. **With RLS now enabled on all 8 tables (2026-06-17 for the original 7, 2026-09-21 for the newly-discovered `hours_backup_causey_20260916`), the anon key alone no longer grants read/write access — a valid authenticated session is required.** ~~Note, however, that the policies are **role-scoped** (`(select auth.role()) = 'authenticated'`), **not user-scoped**, so **any** authenticated account has full access to all rows. The practical guard is therefore an explicit dependency: **the Supabase project must have self-signup disabled and exactly one user account.** As long as that holds, the anon key in the repo grants nothing without a login. Risk level: **low for current single-account usage; the role-scoped policies must be rewritten to `auth.uid()`-scoped before any second user or external sharing (see Known Issues).**~~ **SUPERSEDED 2026-09-21:** the policies are now **user-scoped** — `USING ((select auth.uid()) = 'ea7ec416-3d46-4714-a188-c0f50a63ddcb')`, Lucas's account UUID — so a second authenticated account would no longer automatically inherit access. See [Supabase RLS — Stray Backup Table Secured + All Policies Rewritten to User-Scoped (2026-09-21)](#supabase-rls-stray-backup-table-secured-all-policies-rewritten-to-user-scoped-2026-09-21). Risk level: **low.** New trade-off: the UUID is now hardcoded into 8 policies — if that auth account is ever deleted and recreated, the policies need updating with the new UUID or access breaks entirely.
