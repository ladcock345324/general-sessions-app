...
| `pv_sentence` | text | nullable — what the client faces **if probation is revoked**. Meaningful only when `is_pv` is true. Optional by design and frequently left blank at creation, which is why it is also editable on Case View (its own "Sentence (if known)" section above Notes, shown for every PV case, not only ones that already have a value — otherwise a blank one could never be filled in later). Also rendered in the **right cell** of the PV incident's Incidents row as "Sentence: [text]", or nothing at all when unset. Blank saves as `null`, never `''`. Added with `is_pv`; wired up 2026-08-19. |
...
**Pre-existing data:** one `cases.is_pv = true` row existed from the removed flow (**Richard Bravo, `GS955160`**, no sentence, on incident `4035446a…` whose date/location/description are already all null and whose `is_pv` is `false`). **Left alone pending the user's decision** — flagged, not migrated or deleted. Because its incident is already blank, setting that one incident's `is_pv = true` would make it render identically to a new-flow PV; no other change would be needed.
...
- **Sentence (if known)** textarea with its own Save/Saved confirmation — **PV cases only** (`is_pv`), sits directly **above** Notes, writes `cases.pv_sentence` (blank → `null`). Rendered for every PV case, not only ones with a value already stored: the sentence is usually unknown at creation, so the control has to exist for an empty one or it could never be filled in later.
...
- **NULL text columns (as of 2026-06-17 — ⚠️ the `cases` figure is STALE, see the 2026-07-28 triage table):** ~~`cases`: 2 warrant PDFs on file have NULL `warrant_text` — confirmed scanned/non-OCR'd~~ — **superseded.** As of 2026-07-28 the real count is **4 confirmed scans** (GS1041481, GS1093939, SCE322490, SU26540) — permanently unrecoverable without an OCR step, which the app does not have. The 2 recoverable rows (GS1115757 Johnson, GS1116065 McMillan) were re-uploaded via **Replace Affidavit** and confirmed against the live DB the same day: `warrant_text` populated at 3,971 and 2,778 chars respectively, same recovery method as GS1120368. `clients`: 1 client with NULL `criminal_history_text` but no PDF uploaded (no action needed); `courtroom_documents`: 0 documents uploaded (no action needed)
