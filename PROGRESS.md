...
- **RLS policies are role-scoped, not user-scoped.** All 7 tables allow full access to any authenticated user. Safe only while self-signup is disabled and a single account exists. Must be rewritten to `auth.uid()`-scoped policies before any second user or external sharing.
