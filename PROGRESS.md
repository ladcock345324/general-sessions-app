...
**⚠️ Pre-existing data — `GS955160P1` (Richard Bravo).** Its PV information lives entirely in the deprecated `pv_sentence` (`"DUI (MIS) - Guilty 5/4/2026"`); all four new columns are `null`. **Because nothing reads `pv_sentence` any more, that text now displays nowhere** and the case renders with an empty detail block until the four fields are filled in. **Left untouched — flagged, not migrated**, per the standing rule that data changes are the user's call. It re-enters in seconds through the always-editable Case View fields.
...
- **Incidents** section — **two-column grid, one row per incident, nothing collapsible** (2026-08-10; replaced the click-to-expand accordion):
  ...
  - **PV right cell — the 4-field / 3-line detail block (2026-08-20)**, replacing the old single "Sentence: […]" line. **Line 1** `pv_conviction_date` (via `formatDateDisplay`) · **Line 2** `pv_crime` · **Line 3** `pv_probation_length` and `pv_special_info` joined with `" · "` (the same separator `bondReleaseText()` uses), whichever is set alone if only one is, and the line dropped entirely if neither is. **Each line renders only when it has content**, so the block is 0–3 lines; with nothing filled in the cell is genuinely empty — never a placeholder. `.incidentDescCellPv` centres it vertically so it stays centred at any line count, `gap` tightened to 2px. **`pv_sentence` is not read** — deprecated 2026-08-20
